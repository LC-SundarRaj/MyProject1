...
public class ShipmentTypeUtil {
	...
	public void generatePie(String path, List<Object[]> data) {
		...
		JFreeChart chart = ChartFactory.createPieChart("Shipment Type Modes", dataset, true, true, false);

		//3. Save Chart as image by ChartUtils
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/charts/ShipmentModesPie.jpg"), chart, 500, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void generateBar(String path, List<Object[]> data) {
		...
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/charts/ShipmentModesBar.jpg"), chart, 400, 400);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
